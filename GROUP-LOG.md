...
## Week 7 — Ad (CV + Lead)
- Done: window + encoding ablations (`eval/ABLATION.md`); terminal fallback demo (`src/demo.py`). **Absorbed the sequence slice** (Maheswari absent): SeqLSTM in `src/`, real macro-F1 0.9908, comparison table now 4 models. Took on Ethics 20% — scaffold + sources in `docs/ETHICS.md`, prose next.
- Blocked: the 3 members never started — slices reassigned to me + Amit (Gate 3)
- Next: write the ethics prose myself; email Royce re: non-contributing members
- AI: Claude assisted the LSTM absorption + ethics scaffolding; reviewed line-by-line; logged in AI-USE-LOG

## Week 7 — Amit (Data + Eval)
- Done: **Absorbed the NLP/demo slice** (Nagireddy absent): `src/incident_report.py` + `src/dashboard.py`, wired the report into `src/demo.py` (prints live on alert). Took on Literature — rows 10–14 added [VERIFY].
- Blocked: nothing
- Next: verify the 5 new citations in Zotero; run + screenshot the dashboard for the pitch; Part D draft
- AI: Claude assisted absorbing the generator + wiring; I reviewed and ran it; logged in AI-USE-LOG
