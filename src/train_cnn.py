# train_cnn.py - train the TinyCNN on the REAL hcrl data. Ad (CV + Lead).
# run from the repo root:  python src/train_cnn.py
#
# pipeline: amit's per-file time-ordered split -> non-overlapping windows of
# WINDOW consecutive frames -> frames_to_image -> TinyCNN -> amit's harness.
#
# labelling rule: a window is the attack class if ANY frame inside it is an
# injected frame, else Normal. blunt but standard for window-level ids - and
# it has to be stated in the report because it inflates attack support vs
# frame counts.
#
# two things that keep the split honest at window level:
#   - windows are built per capture file, so a window never spans two files
#     (a window straddling two captures would be a fake pattern)
#   - train windows come only from the train part, test windows only from the
#     test part, so nothing straddles the 70/30 cut either
...
def make_windows(df):
    ...
    # (end timestamp = the moment the detector could actually fire).
    ...
    # max works as "any attack frame present" because Normal is 0 and each
    # file only contains one attack class - no mixed-attack windows possible
    y = labels.max(axis=1)
...
def main():
    ...
    print("Train window classes:", dict(zip(*np.unique(tr_y, return_counts=True))))
    print("Test window classes: ", dict(zip(*np.unique(te_y, return_counts=True))))

    # cap the train windows per class so 3 cpu epochs stay in coffee-break
    # territory. test set is NOT capped - full coverage or the numbers lie.
    keep = []
    ...
    print(f"Training TinyCNN: {EPOCHS} epochs, batch {BATCH}, CPU")
    model.train()
    ...
    # everything reports through amit's harness so the comparison table is
    # apples to apples - one metrics implementation for every model
    ...
    with open("eval/results_cnn.md", "w", encoding="utf-8") as f:
        ...
        f.write(f"- Train: {len(ytr):,} windows (capped stratified sample; "
                f"{CAP_NORMAL} Normal / {CAP_ATTACK} per attack)\n")
        ...
        f.write("Latency = attack onset -> end of first correctly-flagged window, "
                "so the window length itself bounds the best case.\n\n")
